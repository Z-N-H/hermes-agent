...
def _get_cute_tool_message(
    ...
    if tool_name == "web_search":
        return _wrap(f"┊ {NerdFontIcons.get('fa-magnifying_glass')} search    {_trunc(args.get('query', ''), 42)}  {dur}")
    if tool_name == "web_extract":
        urls = args.get("urls", [])
        if urls:
            url = _display_url(urls[0] if isinstance(urls, list) else urls)
            if not url:
                return _wrap(f"┊ {NerdFontIcons.get('fa-file')} fetch     pages  {dur}")
            domain = url.replace("https://", "").replace("http://", "").split("/")[0]
            extra = f" +{len(urls)-1}" if isinstance(urls, list) and len(urls) > 1 else ""
            return _wrap(f"┊ {NerdFontIcons.get('fa-file')} fetch     {_trunc(domain, 35)}{extra}  {dur}")
        return _wrap(f"┊ {NerdFontIcons.get('fa-file')} fetch     pages  {dur}")
    if tool_name == "terminal":
        return _wrap(f"┊ {NerdFontIcons.get('fa-laptop')} $         {_trunc(build_tool_preview(tool_name, args) or args.get('command', ''), 42)}  {dur}")
    if tool_name == "process":
        action = args.get("action", "?")
    ...
    if tool_name == "read_file":
        return _wrap(f"┊ {NerdFontIcons.get('fa-book_open')} read      {_trunc(build_tool_preview(tool_name, args) or args.get('path', ''), 42)}  {dur}")
    if tool_name == "write_file":
    ...
    if tool_name == "skill_view":
        ...
        return _wrap(f"┊ {NerdFontIcons.get('fa-book')} skill     {_trunc(label, 44)}  {dur}")
    ...
    if tool_name == "delegate_task":
        ...
        if tasks and isinstance(tasks, list):
            ...
            return _wrap(f"┊ {NerdFontIcons.get('fa-shuffle')} delegate  {count_label}x: {_trunc(detail, 35)}  {dur}")
...
def get_cute_tool_message(
    ...
    try:
        ...
        return f"┊ {ICON_BOLT} {safe_name:9} completed  {safe_duration}"
