...
**Creating tasks without an assignee.** Tasks created via the dashboard or `kanban_create` without an `assignee` parameter land in `triage` and never reach the dispatcher. Always pass `assignee="<profile>"` when creating tasks. If a task is already stuck in `triage`, use `hermes kanban assign <task_id> <profile>` then `hermes kanban promote <task_id>`.
...
## Troubleshooting tasks that never start

Before debugging a worker, check whether the task reached `running` at all. The most common "nothing is happening" causes are upstream of the worker:

**Task stuck in `triage` with no assignee.** Tasks created via the dashboard (or via `kanban_create` without an `assignee` parameter) land in `triage` status. The dispatcher only picks up tasks in `ready` status that have an assignee set. Fix:
...
If the task is already `ready` (it may have been auto-promoted), `promote` will report that it's not applicable — that's fine, the missing piece was just the assignee.

**Dispatcher not running.** Even a properly assigned `ready` task goes nowhere if no dispatcher process is alive. The dispatcher runs inside the gateway by default (`kanban.dispatch_in_gateway: true`). Check:

```bash
ps aux | grep 'hermes.*gateway' | grep -v grep
```

If nothing shows up, start the gateway:

```bash
hermes gateway run          # foreground
# or: hermes gateway install && hermes gateway start   # as a service (non-WSL)
```

On WSL, `hermes gateway install` warns that systemd services may not survive WSL restarts. Either enable `systemd=true` in `/etc/wsl.conf` for service persistence, or run `hermes gateway run` in a tmux session / background process.

**Verify the task is actually running after fix:**

```bash
hermes kanban show <task_id> --json | python3 -c "import sys,json; t=json.load(sys.stdin)['task']; print(f'Status: {t[\"status\"]}, Assignee: {t[\"assignee\"]}, Runs: {len(json.load(sys.stdin).get(\"runs\",[]))}')"
```

The task lifecycle is: `triage` → `ready` → `running` → `done` / `blocked`. Only `ready` + assigned tasks get dispatched.
